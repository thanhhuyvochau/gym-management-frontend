...
const ManagerManagement = () => {
  ...
  return (
    <Box className='children-box' component={'div'} height={'100%'}>
      <Typography className='mt-8 mb-16' variant='h3' py={2}>
        Member Management
      </Typography>
      {<MUIDataTable title='Gym Owner Management' data={mockUsers} columns={columns} options={options} />}
      {statusModal === 'view' && selectedMember && (
        <MemberCard onClickClose={handleClickClose} member={selectedMember!}></MemberCard>
      )}
    </Box>
  );
};
